...
// ✅ 先把 NOTION_TOKEN 拉出來做 runtime 檢查，並把型別縮窄成 string
const NOTION_TOKEN = process.env.NOTION_TOKEN;
if (!NOTION_TOKEN) {
  throw new Error("Missing NOTION_TOKEN (請在 Vercel 專案環境變數設定 NOTION_TOKEN)");
}

export const notion = new Client({ auth: NOTION_TOKEN });

export const DB = {
  // 這三個用 ?? "" 可以同時滿足 TS 型別，又不會在編譯期噴錯
  KNOWLEDGE: process.env.DB_KNOWLEDGE ?? "",
  VOCAB: process.env.DB_VOCAB ?? "",
  NOTES: process.env.DB_NOTES ?? ""
};

// ---- RichText / Blocks → HTML ----
  ...
  do {
    ...
    for (const b of r.results as any[]) {
      ...
      if (t === "paragraph") parts.push(`<p>${richToHTML(b.paragraph.rich_text)}</p>`);
        ...
        const code = b.code.rich_text?.map((x: any) => x.plain_text).join("") || "";
        parts.push(`<pre><code class="language-${lang}">${escapeHtml(code)}</code></pre>`);
      }
    }
  } while (cursor);
...
export async function queryKnowledge({
  ...
  limit = 10,
  cursor
}: { subjects?: string[], q?: string, limit?: number, cursor?: string }) {
  const filter: any = { and: [] as any[] };
  if (subjects.length) filter.and.push({ or: subjects.map(s => ({ property: "Subject", multi_select: { contains: s } })) });
...
async function cleanKnowledge(p: any) {
  ...
  let detail = "";
  if (props?.Detail?.rich_text?.length) detail = richToHTML(props.Detail.rich_text);
  else detail = await blocksToHTML(p.id);
